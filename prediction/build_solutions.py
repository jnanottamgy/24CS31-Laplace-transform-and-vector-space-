...
.sub{font-size:11pt;color:#4a5361;margin-bottom:18px}
.sec{background:#1d2433;color:#fff;padding:10px 15px;border-radius:4px;margin:0 0 4px;font-size:14.5pt;font-weight:700}
.secmeta{font-size:10pt;color:#5b6472;margin:0 0 16px}
.lbl{font-size:9.5pt;font-weight:700;letter-spacing:.7px;color:#5b6472;margin:20px 0 5px;text-transform:uppercase}
.qbox{border:2px solid #1d2433;border-radius:5px;padding:12px 16px;margin:0 0 4px;background:#fbfcfd}
...
.steps>li::before{content:counter(st);position:absolute;left:0;top:1px;width:23px;height:23px;
  background:#1d2433;color:#fff;border-radius:50%;text-align:center;font-size:10pt;line-height:23px;font-weight:700}
.steps>li b.t{display:block;color:#1d2433;margin-bottom:2px}
.eq{margin:8px 0 8px 4px}
.chk{background:#f2faf5;border:1.6px solid #0d7a45;border-radius:5px;padding:11px 16px;margin:0}
.chk b{color:#0d5c34}
.warn{background:#fdf6f6;border-left:5px solid #a04545;border-radius:0 4px 4px 0;padding:11px 16px;margin:0}
.warn b{color:#8a2f2f}
...
table.t th{background:#39414f;color:#fff;text-align:left;padding:5px 9px;font-size:9.5pt}
...
.warn,.chk,.fbox,.qbox,.why{page-break-inside:avoid}
.steps>li{page-break-inside:avoid}
.lbl{page-break-after:avoid}
.sec,.secmeta{page-break-after:avoid}
...
def sec(n, title, meta):
    w(f'<div class="brk"></div><div class="sec">{n}. &nbsp;{title}</div><div class="secmeta">{meta}</div>')
...
# ================= 1. UNITARY / HERMITIAN =================
sec(1,"Unitary and Hermitian check","Unit V &middot; Q10b &middot; 5 marks &middot; 8 minutes to learn")
...
# ================= 2. PROVE T IS LINEAR =================
sec(2,"Prove <i>T</i> is linear, and find images","Unit III &middot; Q5b &middot; 7 marks &middot; 12 minutes")
blk("The question","qbox",
...
# ================= 3. LEAST SQUARES =================
sec(3,"Least-squares solution of <i>Ax</i> = <i>b</i>","Unit IV &middot; Q8b &middot; 10 marks &middot; 20 minutes")
blk("The question","qbox",
...
# ================= 4. t^n PROOF =================
sec(4,"Prove the <i>t<sup>n</sup></i>-multiplication rule","Unit I &middot; Q1c or Q2b &middot; 7 marks &middot; 15 minutes")
blk("The question","qbox",
...
# ================= 5. HEAVISIDE =================
sec(5,"Piecewise function via Heaviside, then its Laplace transform","Unit II &middot; Q3c &middot; 7 marks &middot; 20 minutes")
blk("The question","qbox",
...
# ================= 6. sin sqrt t =================
sec(6,f"Given <i>L</i>{{sin&nbsp;{sq}<i>t</i>}}, show the cos&nbsp;{sq}<i>t</i> result","Unit I &middot; Q1b &middot; 4 marks &middot; 12 minutes")
blk("The question","qbox",
...
# ================= 7. DEFINITIONS =================
sec(7,"The 2-mark definitions","Units I and II &middot; Q1a, Q2a, Q3a, Q4a &middot; 2 marks each &middot; 10 minutes")
blk("The question","qbox",
 "One of these opens <b>every</b> question in Units I and II, so you get one whichever you choose:<br><br>"
 "&bull;&nbsp; Define the Laplace transform of a function.<br>"
 "&bull;&nbsp; Write the Laplace transform of a periodic function.<br>"
 "&bull;&nbsp; Define a periodic function with an example.<br>"
...
blk("What to write &mdash; learn these five answers word for word","fbox",
 f"<b>1. Laplace transform.</b> For a function <i>f</i>(<i>t</i>) defined for <i>t</i> &ge; 0,"
 f'<div class="eq"><i>L</i>{{<i>f</i>(<i>t</i>)}} = <i>F</i>(<i>s</i>) = {INT}<sub>0</sub><sup>{inf}</sup>'
 f'<i>e</i><sup>&minus;<i>st</i></sup><i>f</i>(<i>t</i>)&thinsp;<i>dt</i></div>'
 f"provided the integral converges.<br><br>"
 f"<b>2. Periodic function.</b> <i>f</i> is periodic with period <i>T</i> if <i>f</i>(<i>t</i>+<i>T</i>) = <i>f</i>(<i>t</i>) "
 f"for all <i>t</i>. Example: sin&nbsp;<i>t</i>, with period 2{pi}.<br><br>"
 f"<b>3. Laplace transform of a periodic function</b> of period <i>T</i>:"
 f'<div class="eq"><i>L</i>{{<i>f</i>(<i>t</i>)}} = {F("1","1 &minus; <i>e</i><sup>&minus;<i>sT</i></sup>")}'
 f'{INT}<sub>0</sub><sup><i>T</i></sup><i>e</i><sup>&minus;<i>st</i></sup><i>f</i>(<i>t</i>)&thinsp;<i>dt</i></div>'
 f"<b>4. Unit step (Heaviside) function.</b>"
 f'<div class="eq"><i>u</i>(<i>t</i>&minus;<i>a</i>) = 0 &nbsp;for <i>t</i> &lt; <i>a</i>, &nbsp;&nbsp; '
 f'<i>u</i>(<i>t</i>&minus;<i>a</i>) = 1 &nbsp;for <i>t</i> &ge; <i>a</i></div>'
 f"Graph: flat along zero, then a vertical jump of height 1 at <i>t</i> = <i>a</i>, then flat at 1. "
 f"Also useful: <i>L</i>{{<i>u</i>(<i>t</i>&minus;<i>a</i>)}} = <i>e</i><sup>&minus;<i>as</i></sup>/<i>s</i>.<br><br>"
 f"<b>5. Dirac-delta (unit impulse) function.</b> {'&#948;'}(<i>t</i>&minus;<i>a</i>) is the limit of a rectangular pulse of "
 f"width {'&#949;'} and height 1/{'&#949;'} centred at <i>t</i> = <i>a</i>, as {'&#949;'} {ARR} 0. It satisfies"
 f'<div class="eq">{"&#948;"}(<i>t</i>&minus;<i>a</i>) = 0 for <i>t</i> &ne; <i>a</i>, &nbsp;&nbsp; and &nbsp;&nbsp; '
 f'{INT}<sub>&minus;{inf}</sub><sup>{inf}</sup>{"&#948;"}(<i>t</i>&minus;<i>a</i>)&thinsp;<i>dt</i> = 1</div>'
 f"Graph: a single vertical arrow at <i>t</i> = <i>a</i>, labelled with area 1. "
 f"Also useful: <i>L</i>{{{'&#948;'}(<i>t</i>&minus;<i>a</i>)}} = <i>e</i><sup>&minus;<i>as</i></sup>.")
blk("Watch out","warn",
 "<ul><li>These are worth <b>4 marks per paper</b> (one in Unit I, one in Unit II) for about five sentences. "
 "Do not skip them because they look trivial.</li>"
 "<li>When the question says &ldquo;represent graphically&rdquo; or &ldquo;sketch&rdquo;, <b>draw the graph</b>. "
 "Half the mark is the picture. Label the axes and the point <i>t</i> = <i>a</i>.</li>"
 "<li>For the Laplace transform definition, add &ldquo;provided the integral converges&rdquo;. It shows you know it "
 "does not always exist.</li></ul>")

# ================= 8. ODEs =================
sec(8,"Solving ODEs and systems of ODEs by Laplace transforms","Unit II &middot; Q3d or Q4d &middot; 7 marks &middot; 22 minutes")
blk("The question","qbox",
...
# ================= 9. SVD =================
sec(9,"Singular Value Decomposition","Unit V &middot; Q9c &middot; 10 marks &middot; 22 minutes")
...
sec(10,"Orthogonal diagonalization","Unit V &middot; Q10c &middot; 10 marks &middot; 25 minutes")
blk("The question","qbox",
...
# ================= 11. SPAN / BASIS =================
sec(11,"Span, basis and linear independence","Unit III &middot; Q5a &middot; 6 marks &middot; 12 minutes")
blk("The question","qbox",
...
# ================= 12. ROTATION PROOF =================
sec(12,"The rotation-operator proof","Unit III &middot; Q5c &middot; 7 marks &middot; 12 minutes")
blk("The question","qbox",
...
sec(13,"Positive-definite check","Unit V &middot; Q10a &middot; 5 marks &middot; 8 minutes &middot; overtime item")
blk("The question","qbox",
...
# ================= 14. FOUR SUBSPACES =================
sec(14,"The four fundamental subspaces","Unit IV &middot; Q7c &middot; 7 marks &middot; 15 minutes &middot; overtime item")
blk("The question","qbox",
...
sec(15,"Complete solution of <i>Ax</i> = <i>b</i>","Unit IV &middot; Q7a &middot; 6 marks &middot; 15 minutes &middot; overtime item")
blk("The question","qbox",
...
# ================= 16. COMPOSITION =================
sec(16,"Composition of matrix transformations","Unit III &middot; Q6a &middot; 6 marks &middot; 12 minutes &middot; overtime item")
blk("The question","qbox",
