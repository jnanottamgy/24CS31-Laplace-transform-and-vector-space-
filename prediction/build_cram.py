...
&nbsp;Frequency key: <span class="lk">&nbsp;4/4&nbsp;</span> in all four papers &nbsp;
<span class="hi">&nbsp;3/4&nbsp;</span> &nbsp; <span class="md">&nbsp;2/4&nbsp;</span> &nbsp;
<span class="lo">&nbsp;1/4&nbsp;</span></div>""")
...
def unit(n, title, rule, pick, rows):
    ...
    w('<table class="g"><tr><th style="width:4%" class="c">#</th><th>Question &mdash; write this</th>'
      '<th class="c" style="width:6%">Marks</th><th class="c" style="width:13%">Section</th>'
      '<th class="c" style="width:6%">Freq</th></tr>')
    for i,(q,mk,sec,fq,cls) in enumerate(rows,1):
        sc = "sec" if cls in ("lk","hi") and "OTHER" not in sec else "sec2"
        w(f'<tr><td class="c"><b>{i}</b></td><td>{q}</td><td class="c"><b>{mk}</b></td>'
          f'<td class="{sc}">{sec}</td><td class="{cls}">{fq}</td></tr>')
    w("</table>")
...
unit("I","Laplace Transforms","20 marks &middot; a(2) + b(4) + c(7) + d(7)",
 ...
 [(f"<b>Prove</b> &nbsp;<i>L</i>{{<i>t<sup>n</sup>f</i>(<i>t</i>)}} = (&minus;1)<sup><i>n</i></sup> "
   ...
   "<span style='color:#0d5c34'><b>&larr; THE trigger. Do this first.</b></span>","07","Q1c or Q2b","4/4","lk"),
  (f"Given <i>L</i>{{sin&nbsp;{sq}<i>t</i>}} = "+F(f"{sq}{pi}","2<i>s</i><sup>3/2</sup>")+
   ...
   +F(f"{sq}{pi}","<i>s</i><sup>1/2</sup>")+"<i>e</i><sup>&minus;1/4<i>s</i></sup>","04","Q1b &mdash; with #1","4/4","lk"),
  (f"<b>Evaluate</b> &nbsp;(i) <i>L</i>{{<i>t e</i><sup>&minus;4<i>t</i></sup> sin&nbsp;3<i>t</i>}} &nbsp;&nbsp; "
   ...
   f"{INT}<sub>s</sub><sup>{inf}</sup><i>F</i>)</span>","07","Q1d or Q2c","4/4","lk"),
  (f"<b>Evaluate</b> &nbsp;(i) {INT}<sub>0</sub><sup>{inf}</sup> "+F("<i>e</i><sup>&minus;<i>t</i></sup> sin&nbsp;<i>t</i>","<i>t</i>")
   +f"<i>dt</i> &nbsp;&nbsp;(<i>ans</i> {pi}/4) &nbsp;&nbsp;(ii) <i>L</i>{{<i>t</i>&thinsp;sin&nbsp;3<i>t</i>&thinsp;cos&nbsp;2<i>t</i>}}","07","Q1d","3/4","hi"),
  ("<b>Periodic function.</b> Triangular wave, period 2<i>a</i>: &nbsp;<i>f</i>(<i>t</i>) = "
   ...
   f"<i>E</i>&thinsp;sin({om}<i>t</i>) on (0,{pi}/{om}), 0 after","07","<b>OTHER</b> question","4/4","lk"),
  (f"<b>Obtain</b> &nbsp;<i>L</i>{{({sq}<i>t</i> + 1/{sq}<i>t</i>)<sup>3</sup>}} &nbsp;&nbsp; "
   f"<span style='color:#8a5a12'>(expand first, then term-by-term)</span>","04","Q2b","2/4","md"),
  ("<b>Definition (2 marks &mdash; free).</b> &ldquo;Write the Laplace transform of a periodic function&rdquo; = "
   ...
   " &nbsp;&bull;&nbsp; or &ldquo;Define the Laplace transform&rdquo;","02","Q1a &amp; Q2a","4/4","lk"),
 ])
...
unit("II","Application of Laplace Transforms","20 marks &middot; a(2) + b(4) + c(7) + d(7)",
 ...
 [("<b>Express in terms of the Heaviside function and find its LT:</b> &nbsp;<i>f</i>(<i>t</i>) = "
   ...
   +" <span style='color:#0d5c34'><b>&larr; THE trigger. Same values twice.</b></span>","07","Q3c","4/4","lk"),
  ("<b>Solve by Laplace transforms</b> (one of these, in the <i>same</i> question as #1):<br>"
   ...
   "07","Q3d / Q4d &mdash; with #1","4/4","lk"),
  ("<b>Convolution theorem.</b> &nbsp;"+inv+"{"+F("<i>s</i><sup>2</sup>","(<i>s</i><sup>2</sup>+<i>a</i><sup>2</sup>)(<i>s</i><sup>2</sup>+<i>b</i><sup>2</sup>)")
   ...
   "07","<b>OTHER</b> question","4/4","lk"),
  ("<b>Evaluate</b> &nbsp;"+inv+"{ log "+F("<i>s</i><sup>2</sup>+1","<i>s</i>(<i>s</i>+1)")+" } &nbsp;&nbsp;"
   "<span style='color:#8a5a12'>differentiate <i>F</i>, then "+inv+"{<i>F</i>&prime;} = &minus;<i>t f</i>(<i>t</i>)</span>","04","Q3b","3/4","hi"),
  ("<b>Find</b> &nbsp;"+inv+"{"+F("3<i>s</i>+2","<i>s</i><sup>2</sup>&minus;<i>s</i>&minus;2")+"} &nbsp;&mdash; partial fractions. "
   "Every paper carries a short inverse LT in <i>both</i> options.","04","Q4b","4/4","lk"),
  ("<b>Definition (2 marks &mdash; free).</b> Define the unit step function + graph &nbsp;&bull;&nbsp; "
   "Define the Dirac-delta function + sketch","02","Q3a &amp; Q4a","4/4","lk"),
 ])
...
unit("III","Vector Space and Linear Transformation","20 marks &middot; a(6) + b(7) + c(7)",
 ...
 [(f"<b>Matrix of a composition + image of a point.</b> Reflection in the <i>x</i>-axis, then rotation {pi}/2, "
   ...
   "Reflect in <i>x</i> = diag(1,&minus;1); in <i>y</i> = diag(&minus;1,1).</span>","06","Q6a","4/4","lk"),
  (f"<b>Transition matrix.</b> <i>B</i> = {{(1,&nbsp;2), (3,&nbsp;&minus;1)}}, &nbsp;<i>B</i>&prime; = {{(3,&nbsp;1), (5,&nbsp;2)}} of <i>R</i><sup>2</sup>. "
   ...
   "<span style='color:#0d5c34'><b>Same bases twice running.</b></span>","07","Q6b &mdash; with #1","4/4","lk"),
  (f"<b>Kernel and range + verify rank&ndash;nullity</b> for &nbsp;<i>A</i> = {A_KER} &nbsp;&nbsp;"
   ...
   "<i>T</i>(<i>x</i>,<i>y</i>) = (3<i>x</i>, <i>x</i>&minus;<i>y</i>, <i>y</i>).","07","Q6c &mdash; with #1, #2","4/4","lk"),
  (f"<b>Prove <i>T</i> is linear + find images.</b> <i>T</i>:<i>P</i><sub>2</sub>{ARR}<i>P</i><sub>2</sub>, "
   ...
   f"<span style='color:#8a5a12'>OR <i>T</i>(<i>x</i>,<i>y</i>) = (3<i>x</i>+<i>y</i>, 2<i>y</i>, <i>x</i>&minus;<i>y</i>); images of (1,2), (2,&minus;5)</span>","07","<b>Q5b</b> &mdash; all 4 papers","4/4","lk"),
  ("<b>Span / linear combination / independence / basis.</b> e.g. is a set of four 2&times;2 matrices a basis of "
   ...
   "or: does {(1,2,3), (&minus;1,&minus;1,0), (2,5,4)} span <i>R</i><sup>3</sup>?","06","Q5a","4/4","lk"),
  (f"<b>Prove</b> <i>T</i>(<i>x</i>) = <i>Ax</i> rotates <i>x</i> through {th} about the origin if "
   f"<i>A</i> = {A_ROT} &nbsp;&mdash; bookwork escape hatch if Q6 looks ugly.","07","Q5c","2/4","md"),
 ])
...
unit("IV","Orthogonal Projections","20 marks &middot; <b>Q7 = 6+7+7</b> &nbsp;|&nbsp; <b>Q8 = 10+10</b>",
 ...
 [(f"<b>QR factorization.</b> Mar&nbsp;24 used {A_QR}. Matrix changes every paper &mdash; own the method: "
   "Gram&ndash;Schmidt the columns, normalise &rarr; <i>Q</i>, then <b><i>R</i> = <i>Q</i><sup>T</sup><i>A</i></b>.","10","<b>Q8a</b>","4/4","lk"),
  (f"<b>Least-squares solution of <i>AX</i> = <i>b</i>:</b> &nbsp;<i>A</i> = {A_LS} &nbsp;<i>b</i> = {B_LS} "
   "<span style='color:#0d5c34'><b>&larr; identical values, twice.</b></span> Solve <i>A</i><sup>T</sup><i>A</i>&#770;<i>x</i> = <i>A</i><sup>T</sup><i>b</i>.","10","<b>Q8b</b> &mdash; with #1","4/4","lk"),
  (f"<b>Dimension and basis for the four fundamental subspaces</b> of &nbsp;<i>A</i> = {A_SUB} "
   ...
   "<span style='color:#8a5a12'>row&nbsp;3 = row&nbsp;1 &rArr; rank 2; dim&nbsp;nul = 2, dim&nbsp;left-nul = 1</span>","07","<b>Q7</b> &mdash; always","4/4","lk"),
  ("<b>Complete solution of <i>Ax</i> = <i>b</i>.</b> Mar&nbsp;24: <i>x</i>+3<i>y</i>+3<i>z</i>=1, "
   ...
   "Answer as <i>x</i> = <i>x<sub>p</sub></i> + <i>c</i><sub>1</sub><i>n</i><sub>1</sub> + &hellip;","06","Q7a","3/4","hi"),
  (f"<b>Orthogonal projection.</b> <i>y</i> = {V(['2','3'])}, <i>u</i> = {V(['4',sp+'7'])}. Find proj of <i>y</i> onto <i>u</i>; "
   ...
   "<span style='color:#8a5a12'>proj = ((<i>y</i>&middot;<i>u</i>)/(<i>u</i>&middot;<i>u</i>))<i>u</i> &mdash; 5 minutes, 7 marks</span>","07","Q7b","1/4","lo"),
  ("<b>Gram&ndash;Schmidt orthonormal basis</b> &mdash; e.g. span{(1,1,1,1), (0,1,1,1), (0,0,1,1)}. "
   "You already know this from QR: just stop before forming <i>R</i>.","10","Q8a substitute","1/4","lo"),
 ])
...
unit("V","Applications of Eigenvalue Decomposition","20 marks &middot; a(5) + b(5) + c(10)",
 ...
 [(f"<b>Orthogonally diagonalize</b> &nbsp;<i>A</i> = {A_OR} "
   ...
   "on their own. Normalise &rarr; <i>P</i>, then <i>P</i><sup>T</sup><i>AP</i> = <i>D</i>.</span>","10","Q10c","4/4","lk"),
  (f"<b>Is</b> &nbsp;<i>A</i> = {A_PD} <b>positive definite?</b> "
   "<span style='color:#8a5a12'>minors 1, 0, 0 &rarr; <b>positive semi-definite, NOT positive definite</b>. Say so explicitly.</span>","05","Q10a &mdash; with #1","4/4","lk"),
  (f"<b>Is</b> &nbsp;<i>A</i> = {A_UN} <b>unitary?</b> Show <i>A</i><sup>*</sup><i>A</i> = <i>I</i> &nbsp;(<b>yes</b>) "
   f"&nbsp;&nbsp;<span style='color:#8a5a12'>OR the Hermitian version:</span> is {A_HE} Hermitian? &nbsp;(<b>yes</b>)","05","Q10b &mdash; with #1, #2","4/4","lk"),
  (f"<b>Find the SVD of</b> &nbsp;<i>A</i> = {A_SV} "
   f"<span style='color:#8a5a12'>&nbsp;<i>A</i><sup>T</sup><i>A</i> = {M([['10',sp+'8'],[sp+'8','10']])}, "
   f"eigenvalues 18 and 2 &rArr; singular values 3{sq}2 and {sq}2</span>","10","<b>OTHER</b> question (Q9c)","4/4","lk"),
  ("<b>Quadratic form &rarr; no cross-product terms.</b> <i>Q</i>(<i>x</i>) = <i>x</i><sub>1</sub><sup>2</sup> "
   ...
   "<span style='color:#8a5a12'>Build the symmetric matrix, orthogonally diagonalise, substitute <i>x</i> = <i>Py</i>.</span>","05","Q9a","2/4","md"),
  (f"<b>Steady state of the Markov matrix</b> &nbsp;<i>A</i> = {A_MK} &nbsp;&nbsp;"
   "<span style='color:#8a5a12'>solve (<i>A</i>&minus;<i>I</i>)<i>x</i> = 0, scale so entries sum to 1</span>","05","Q9b","1/4","lo"),
  ("<b>Diagonalize and hence find <i>A<sup>n</sup></i></b> &nbsp;(<i>A<sup>n</sup></i> = <i>PD<sup>n</sup>P</i><sup>&minus;1</sup>). "
   "Appeared 3 papers straight, then <b>skipped in Mar&nbsp;24</b> &mdash; which makes it <b>overdue</b>, not safe.","10","Q9 (10-mark slot)","3/4","hi"),
 ])
